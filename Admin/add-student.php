...
if(isset($_POST['add']))
{
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],"img/profile_pic/".$_FILES["image"]["name"]);
		$query="insert into student(firstname, middlename, lastname, email,phone,course,image) values(?,?,?,?,?,?,?)";
$stmt = $conn->prepare($query);
$rc=$stmt->bind_param('sssssss',$first,$middle,$last,$email,$phone,$course,$image);
$stmt->execute();
echo"<script>alert('Student account Successfully created');</script>";
}
?>
<ol class="breadcrumb">
